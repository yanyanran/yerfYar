...
func TestCutToLastMessage(t *testing.T) {
	...
	if !bytes.Equal(gotTruncated, wantTruncated) || !bytes.Equal(gotRest, wantRest) {
		t.Errorf("cutToLastMessage(%q): got %q, %q; want %q, %q", string(res), string(gotTruncated),
...
func TestCutToLastMessageErrors(t *testing.T) {
	...
	if err == nil {
		t.Errorf("TestCutToLastMessageErrors(%q): got no errors; want an error", string(res))
	}
}
